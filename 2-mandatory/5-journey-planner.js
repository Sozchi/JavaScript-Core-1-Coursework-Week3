...
{
  let londonT = arr.slice( 1, arr.length );
  return londonT;
}
...
*/
function isAccessibleByTransportMode( modeOfT, toChoose )
{
  if ( toChoose.includes( modeOfT ) )
  {
    return true;
  } else
  {
...
{
  return locations.filter( ( a ) => a.includes( transportMode ) ).map( ( a ) => a[0] );
 }
